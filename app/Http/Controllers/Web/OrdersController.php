...
class OrdersController extends Controller
{
    ...
    public function shurjoPayStore($price)
    {
        $shurjopay_service = new ShurjopayService();
        $shurjopay_service->generateTxId(uniqid('YFIT-'));
...
//        dd($shurjopay_service);
    ...
    {
        $order_id = $request['tx_id'];

        if ('000' === $request['sp_code']) {
            Session(['shurjo_pay' => 'success']);
            return redirect('/checkout');

        } else if ('001' === $request['sp_code']) {
            return view('web.shurjopay.payment-failed');
        }
    }
    ...
    public function thankyou(Request $request)
    {
        ...
        Session::forget('netPrc');
        Session::forget('shipP');
        // Session::forget('products_options_values');
    ...
    public function getPaymentMethods()
    {
        ...
        $payments_setting = $this->order->payments_setting_for_shurjo_pay();


        if ($payments_setting['server_url']->environment == '0') {
            $shurjo_pay_enviroment = 'Test';
        } else {
            $shurjo_pay_enviroment = 'Live';
        }

        $shurjo_pay = array(
            'environment' => $shurjo_pay_enviroment,
            'payment_currency' => Session::get('currency_code'),
            'public_key' => '',
            'name' => $payments_setting['server_url']->name,
            'active' => $payments_setting['server_url']->status,
            'payment_method' => $payments_setting['server_url']->payment_method,
        );
    ...
    {
        $shippingPrice = DB::table('zones')->select('shipping_price')->where('zone_id', '=', $id)->first();
        $shipPrice = $shippingPrice->shipping_price;
        return response()->json($shipPrice);
    }    

    public function set_price_to_session(Request $request)
    {
        Session::forget('cartPrice');
        Session::put('netPrc', $request->netPrc);
        Session::put('shipP', $request->shipP);
    }

    public function set_cart_price_to_session(Request $request)
    {
        Session::put('cartPrice', $request->cartPrice);
    }
