...
class ProductsController extends Controller
{
    ...
    public function productDetail(Request $request)
    {
        ...
        if(!empty($products) and count($products)>0){
            ...
            $attribute_options = \App\Models\Core\ProductAttribute::where('product_id', $product_id)->get();
            // dd($attribute_options);
            foreach ($attribute_options as $value) {
                $attribute_id = $value->attribute_id;
                $attribute = \App\Models\Core\Attribute::find($attribute_id);
                array_push($tempAttributes, $attribute);

            }
            $attributes = array_unique($tempAttributes);
        ...
        return view("web.detail", ['title' => $title, 'final_theme' => $final_theme], compact('result', 'attribute_options', 'attributes'));
    }
