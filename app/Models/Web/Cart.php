...
class Cart extends Model
{
    ...
    public function myCart($baskit_id)
    {
        $cart = DB::table('customers_basket')
            ...
            ->join('products_description', 'products_description.products_id', '=', 'customers_basket.products_id')
            ...
            ->LeftJoin('new_customers_basket_attributes', function ($join) {
                $join->on('new_customers_basket_attributes.products_id', '=', 'products.products_id')
                    ->where(function ($query) {
                        $query->where([
                            'new_customers_basket_attributes.is_orders' => 0, 
                            'new_customers_basket_attributes.session_id' => Session::getId(),
                        ]);
                    });
            })
            ->where([
            ...
            ->select('customers_basket.*',
                ...
                'products.products_weight as weight', 'products.products_weight_unit as unit', 'products.products_slug', 'new_customers_basket_attributes.products_options_values', 'new_customers_basket_attributes.products_options_name');
    ...
    public function addToCart($request)
    {
        ...
        // $final_price = substr($request->products_price, 1);
        $original_price = $result['detail']['product_data'][0]->products_price + 0;
        $final_price = $request->products_price;
            ...
            if (count($exist) == 0) {
                ...
                if (!empty($request->option_id)) {
                    ...
                    [
                        ...
                        DB::table('new_customers_basket_attributes')->insert(
                            [
                                'customers_id' => $customers_id,
                                'products_id' => $products_id,
                                'products_options_id' => $request->option_id,
                                'products_options_name' => $request->option_name,
                                'products_options_values' => $request->option_choice,
                                'is_orders' => 0,
                                'session_id' => $session_id,
                            ]);

                } else if(empty($request->option_id)) {
                    $customers_basket_id = DB::table('customers_basket')->insertGetId(
                    [
                        'customers_id' => $customers_id,
                        'products_id' => $products_id,
    ...
    public function updateRecord($customers_basket_id, $customers_id, $session_id, $quantity, $price)
    {
