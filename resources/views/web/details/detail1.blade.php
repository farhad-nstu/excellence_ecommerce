    ...
    <div class="row">
      ...
      <div class="col-12 col-lg-6">
          ...
          <div class="price">  
            ...
            <h5 id="total_price" value="{{ $attribute_options[0]->product_id }}">৳{{ $attribute_options[0]->price }}</h5>
          ...
          <div class="pro-infos">
              ...
              <div class="pro-single-info"><b>@lang('website.Categroy')  :</b>
                ...
                <!-- produtcs attribute strat -->
                <div class="pro-single-info">
                  <span id="quantity" value="{{ $attribute_options[0]->product_id }}"><b>Quantity</b>: {{ $attribute_options[0]->quantity }}</span>
                </div>

                @foreach($attributes as $attribute)
                <div class="row ml-0 py-4">
                  <h5 class="font-weight-bold disabled">{{ $attribute->name }}</h5>&nbsp;&nbsp;&nbsp;
                  @foreach($attribute_options as $attribute_option)
                
                  @if($attribute_option->attribute_id == $attribute->id)

                  <button type="button" id="sizeAtt" class="btn btn-sm btn-success sizeAtt {{ $attribute_option->choice_option }}" onclick="getSize(`{{ $attribute->name }}`, `{{ $attribute_option->choice_option }}`, `{{ $attribute_option->price }}`, `{{ $attribute_option->quantity }}`, `{{ $attribute_option->attribute_id }}`)">

                    {{ $attribute_option->choice_option }}
          ...
          <div class="pro-options row">
            ...
            @foreach($attributes as $attribute)                
              @if($attribute_options[0]->attribute_id == $attribute->id)
                <input type="hidden" name="option_name" id="option_name" value="{{ $attribute->name }}" >
                <input type="hidden" name="option_id" id="option_id" value="{{ $attribute->id }}" >
              @endif
            @endforeach            
            <input type="hidden" name="option_choice" id="option_choice" value="{{ $attribute_options[0]->choice_option }}" >
...
@endif
  ...
  <script>
    ...
    var substringedQuantity = quantity.substring(10);
    ...
    function getSize(att, attSize, price, quantity, id){
      ...
      this.price = price;
      this.option_name = attSize;

      prices = document.getElementById("total_price").innerHTML ="৳"+price;

      document.getElementById("option_name").value = att;
      document.getElementById("option_id").value = id;
      document.getElementById("option_choice").value = attSize;
      document.getElementById("products_price").value = price;
    ...
    function increase(){
      incQty++;
      var original_qty = document.getElementById("quantity").textContent;
      var sub_original_qty = original_qty.substring(10);      
      var qty2 = parseInt(qty) + incQty;

      if(qty2 <= parseInt(sub_original_qty)){
        document.getElementById("qnty").value = qty2;
        var f_price = this.price*qty2;
        document.getElementById("total_price").innerHTML ="৳"+f_price;
      }
      ...
      document.getElementById("products_price").value = f_price;
    ...
    function decrease(){
      ...
      tempQty = document.getElementById("qnty").value;
      finalQty = parseInt(tempQty) - 1;
      console.log(finalQty);

      if(finalQty >= 1){
        document.getElementById("qnty").value = finalQty;
        var f_price = this.price*finalQty;
